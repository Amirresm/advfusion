...
OUTPUT_DIR="/scratch/amirresm/outputs/advfusion/train_advf_codellama_ct_${lang}"
mkdir -p "$OUTPUT_DIR"
rm "$OUTPUT_DIR"/job.log || true
exec > >(tee -a "$OUTPUT_DIR/job.log") 2>&1
pip freeze >"$OUTPUT_DIR/requirements.txt"

model_path="$STORAGE_ROOT/models/meta-llama/CodeLlama-7b"
ds_path="$STORAGE_ROOT/data/ct_dataset/${lang}"

target_adapter_path="/scratch/amirresm/outputs/advfusion/train_adp_codellama_ct_lua"
